...
def conflict_handler(request: Request, exc: FakturoConflictError):
    ...
    return JSONResponse(
        status_code=409, content={"detail": f"{exc.resource_name}: {exc.exc_detail}"}
    )
...
def business_rule_handler(request: Request, exc: BusinessRuleError):
    ...
    return JSONResponse(
        status_code=422, content={"rule_name": exc.rule, "detail": exc.detail}
    )


def ares_not_available_handler(
    request: Request, exc: ARESNotAvailableError
) -> JSONResponse:

    return JSONResponse(status_code=503, content={"detail": "Ares not available"})
