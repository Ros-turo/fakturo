...
class TagRepo(BaseRepo):
    async def create_tag(self, name: str, owner_id: int) -> Tag:
        ...
        tag_exist = await self.db.execute(
    ...
    async def add_tag_to_invoice(
        self, invoice_id: int, tag_id: int, uid: int
    ) -> Invoice | None:
        tag_result = await self.db.execute(
            select(Tag).where(Tag.id == tag_id, Tag.owner_id == uid)
        )
    ...
    async def bulk_upsert_tags(self, tags: list[str], owner_id: int) -> list[Tag]:
        stmt = insert(Tag).values(
            [{"owner_id": owner_id, "name": name} for name in tags]
        )
